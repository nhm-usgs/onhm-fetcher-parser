...
# Grab an example .nc file from Gridmet Thredds server.  Example URL below:
...
with open(r'../DelData/test3.nc', 'wb') as fh:
    fh.write(myfile.content)
    fh.close()
uofi_file = r'../DelData/test3.nc'
ds = xr.open_dataset(uofi_file)
print(ds)
#Read NHM hru shapefiles into geopandas dataframe
print(os.getcwd())
from pathlib import Path
folder = Path(r'../DelData') # assumes working directory is onhm-fetcher-parser
print(folder)
# shapefiles = folder.glob("*_0[1-2].shp")
#glob here because original nhm had set of shapefiles - still works for single shapefile
shapefiles = folder.glob("*geographic*.shp")
gdf = pd.concat([
    gpd.read_file(shp)
...
tcount = 0
with open(r'../DelData/Delaware_weights_hru_v1_0.csv', 'w', newline='') as f:
