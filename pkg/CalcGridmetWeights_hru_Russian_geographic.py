...
with open(r'../RussianData/test3.nc', 'wb') as fh:
    fh.write(myfile.content)
    fh.close()

uofi_file = r'../RussianData/test3.nc'
ds = xr.open_dataset(uofi_file)
print(ds)
#Read NHM hru shapefiles into geopandas dataframe
print(os.getcwd())
from pathlib import Path
folder = Path(r'../RussianData') # assumes working directory is onhm-fetcher-parser
print(folder)
# shapefiles = folder.glob("*_0[1-2].shp")
shapefiles = folder.glob("*2.shp")
gdf = pd.concat([
    gpd.read_file(shp)
...
tcount = 0
with open('../RussianData/Russian_weights_hru_v3b.csv', 'w', newline='') as f:
